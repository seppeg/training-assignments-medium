...
public class BasicConformityMonkeyContext extends BasicSimianArmyContext implements ConformityMonkey.Context {
    ...
    public BasicConformityMonkeyContext() {
        ...
        clusterTracker = createClusterTracker();
        ruleEngine = createAndSetupRuleEngine();
    ...
    private ConformityRuleEngine createAndSetupRuleEngine() {
        ConformityRuleEngine ruleEngine = new ConformityRuleEngine();

        addConfigurationForEureka(ruleEngine);
        ...
        addConfigurationIfNeeded(ruleEngine, "simianarmy.conformity.rule.InstanceTooOld.enabled", new InstanceTooOld(getAwsCredentialsProvider(), (int) configuration().getNumOrElse(
                "simianarmy.conformity.rule.InstanceTooOld.instanceAgeThreshold", 180)));
        addConfigurationIfNeeded(ruleEngine, "simianarmy.conformity.rule.SameZonesInElbAndAsg.enabled", new SameZonesInElbAndAsg(getAwsCredentialsProvider()));
        addConfigurationIfNeeded(ruleEngine, "simianarmy.conformity.rule.InstanceInVPC.enabled", new InstanceInVPC(getAwsCredentialsProvider()));
        addConfigurationIfNeeded(ruleEngine, "simianarmy.conformity.rule.CrossZoneLoadBalancing.enabled", new CrossZoneLoadBalancing(getAwsCredentialsProvider()));

        return ruleEngine;
    }

    private void addConfigurationForEureka(ConformityRuleEngine ruleEngine) {
        ...
        if (eurekaEnabled) {
            ...
            addConfigurationIfNeeded(ruleEngine, "simianarmy.conformity.rule.InstanceIsHealthyInEureka.enabled", new InstanceIsHealthyInEureka(conformityEurekaClient));
            addConfigurationIfNeeded(ruleEngine, "simianarmy.conformity.rule.InstanceHasHealthCheckUrl.enabled", new InstanceHasHealthCheckUrl(conformityEurekaClient));
            addConfigurationIfNeeded(ruleEngine, "simianarmy.conformity.rule.InstanceHasStatusUrl.enabled", new InstanceHasStatusUrl(conformityEurekaClient));
        } else {
            LOGGER.info("Discovery/Eureka is not enabled, the conformity rules that need Eureka are not added.");
        }
    }

    private void addConfigurationIfNeeded(ConformityRuleEngine ruleEngine, String propertyToCheck, ConformityRule ruleToAdd) {
        if (configuration().getBoolOrElse(
                propertyToCheck, false)) {
            ruleEngine.addRule(ruleToAdd);
        }
    }
